...
VEED Skills is a set of independent, self-contained skills that teach an AI agent to call VEED's video APIs (hosted on Fal). Four skills today:
...
Each skill is independent. They don't share state and don't depend on each other. An agent can install any subset.
...
│   └── marketplace.json            # Marketplace listing with 4 skills
├── .codex-plugin/plugin.json       # Codex manifest
├── .cursor-plugin/plugin.json      # Cursor manifest
├── veed-talking-head/SKILL.md
├── veed-talking-head-text/SKILL.md
├── veed-subtitles/SKILL.md
└── veed-background-removal/SKILL.md
```

There is **no root SKILL.md**. The four skills are independent.
